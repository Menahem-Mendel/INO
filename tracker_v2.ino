...
int servoPos = servo_10.read(); // позиция сервопривода
...
void loop()
{
	if (servo_10.attached())
	{
		...
		leftLDR = map(leftLDR, 0, SENSITIVITYBEFORE, 0, SENSITIVITY);
		rightLDR = map(rightLDR, 0, SENSITIVITYBEFORE, 0, SENSITIVITY);
